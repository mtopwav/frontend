...
function AdminReports() {
  ...
  const handlePrintSalesReport = () => {
    ...
    const body =
      '<thead><tr><th class="tc">#</th><th class="tl">Date</th><th class="tl">Customer</th><th class="tl">Method</th><th class="tr">Amount (TZS)</th><th class="tl">Status</th></tr></thead><tbody>' +
      (rows.length
        ? rows.map((p, i) => `<tr><td class="tc">${i + 1}</td><td>${String(p.created_at || '').replace('T', ' ').slice(0, 16)}</td><td>${String(p.customer_name || '—').replace(/</g, '&lt;')}</td><td>${String(p.payment_method || '—').replace(/</g, '&lt;')}</td><td class="tr">${Math.round(Number(p.total_amount) || 0).toLocaleString()}</td><td>${p.status || '—'}</td></tr>`).join('')
        : '<tr><td colspan="6" class="tc">No sales records found</td></tr>') +
      '</tbody>';
