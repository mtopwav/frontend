...
function AdminTransactions() {
  ...
  const totalMobilePayments = approvedPaymentsForTable.reduce(
    (sum, p) => {
      ...
      return method.includes('mobile') || method.includes('mpesa') || method.includes('tigo') || method.includes('airtel')
        ...
        <body>
          <div class="tax-inv-top">
            ...
            <div class="tax-inv-meta">
              ...
              <p><strong>Printed by:</strong> ${(user?.full_name || user?.username || 'Admin').replace(/</g, '&lt;')}</p>
            </div>
          </div>
                        ...
                        <tr key={payment.id}>
                          ...
                          <td>
                            <div className="txn-customer-cell">
                              ...
                              {payment.customer_phone && <div className="txn-detail">{payment.customer_phone}</div>}
                            </div>
                          </td>
                          <td>
                            {payment.items && payment.items.length > 0 ? (
                              <span>
                                {payment.items.map((item) => capitalizeName(item.sparepart_name || 'Unknown')).join(', ')}
                              </span>
                            ) : (
