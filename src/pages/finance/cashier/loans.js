...
<body>
  ...
  if (loading) return <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>Loading loans...</div>;
  if (!user) return null;
    ...
    <div className="payments-container">
      <aside className={`sidebar ${sidebarOpen ? 'open' : 'closed'}`}>
        ...
        <nav className="sidebar-nav">
          ...
          <Link to="/finance/cashier/loans" className={'nav-item ' + (location.pathname === '/finance/cashier/loans' ? 'active' : '')}><FaMoneyBillWave className="nav-icon" /><span>{t.loans || 'Loans'}</span></Link>
